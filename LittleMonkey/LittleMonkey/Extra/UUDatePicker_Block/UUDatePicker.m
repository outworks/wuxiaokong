...
- (void)drawRect:(CGRect)rect
{
    if (self.frame.size.height<216){
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, ScreenWidth, 216);
    }
    ...
    if (self.datePickerStyle == UUDateStyle_YearMonthDayHourMinute){
        if (isIOS7) {
            [self creatValuePointXs:@[@"80",@"135",@"190",@"245",@"300"]
                          withNames:@[NSLocalizedString(@"年", nil),NSLocalizedString(@"月", nil),NSLocalizedString(@"日", nil),NSLocalizedString(@"时", nil),NSLocalizedString(@"分", nil)]];
        }
        return 5;
    }
    if (self.datePickerStyle == UUDateStyle_YearMonthDay){
        if (isIOS7) {
            [self creatValuePointXs:@[@"120",@"200",@"270"]
                      withNames:@[NSLocalizedString(@"年", nil),NSLocalizedString(@"月", nil),NSLocalizedString(@"日", nil)]];
            }
        return 3;
    }
    if (self.datePickerStyle == UUDateStyle_MonthDayHourMinute){
        ...
        [self creatValuePointXs:@[@"90",@"160",@"230",@"285"]
                      withNames:@[NSLocalizedString(@"月", nil),NSLocalizedString(@"日", nil),NSLocalizedString(@"时", nil),NSLocalizedString(@"分", nil)]];
            }
        return 4;
    }
    if (self.datePickerStyle == UUDateStyle_HourMinute){
        if (isIOS7) {
        [self creatValuePointXs:@[[NSString stringWithFormat:@"%f",ScreenWidth * (7/16.0)],[NSString stringWithFormat:@"%f",ScreenWidth * (47/64.0)]]
                      withNames:@[NSLocalizedString(@"时", nil),NSLocalizedString(@"分", nil)]];
            }
        return 2;
    }
...
- (NSString*)getWeekDayWithYear:(NSString*)year month:(NSString*)month day:(NSString*)day
{
    ...
    NSString *weekDay = @"";
    switch (w) {
        case 0: weekDay = NSLocalizedString(@"周日", nil);    break;
        case 1: weekDay = NSLocalizedString(@"周一", nil);    break;
        case 2: weekDay = NSLocalizedString(@"周二", nil);    break;
        case 3: weekDay = NSLocalizedString(@"周三", nil);    break;
        case 4: weekDay = NSLocalizedString(@"周四", nil);    break;
        case 5: weekDay = NSLocalizedString(@"周五", nil);    break;
        case 6: weekDay = NSLocalizedString(@"周六", nil);    break;
        default:break;
    }
    return weekDay;
}
