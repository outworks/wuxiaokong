...
- (void)initializePageControl {
    ...
    NSArray *tempArray = [_datasource numberOfKDCycleBannerView:self];
    
    CGRect pageControlFrame = CGRectMake((self.window.bounds.size.width - 13*tempArray.count - 10)/2.0, CGRectGetMaxY(self.frame)-25, 13*tempArray.count, 25);
    ...
    _pageControl.currentPageIndicatorTintColor = RGB(78, 204, 120);
    _pageControl.pageIndicatorTintColor = RGB(255, 255, 255);
...
- (void)loadData {
    ...
    if (self.isContinuous) {
        ...
        [cycleDatasource addObject:[_datasourceImages objectAtIndex:0]];
    ...
    for (NSInteger i = 0; i < _datasourceImages.count; i++) {
        ...
        if ([imageSource isKindOfClass:[UIImage class]]) {
            //UIImage *imageSoure2 = [self scaleToSize:imageSource size:imageView.frame.size];
            UIImage *imageSoure2 =(UIImage *)imageSource;
            imageView.image = imageSoure2;
