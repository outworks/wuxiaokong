...
- (UIImage *)trackImage
{
    if(_trackImage==nil)
    {
        ...
        else
        {
            ...
            UIImage* image = [self createImageWithColor:[UIColor colorWithRed:79.0/255.0 green:205.0/255.0 blue:116.0/255.0 alpha:1.000]];
            image = [self createImageWithColor:UIColorFromRGB(0xff6948)];
            image = [image resizableImageWithCapInsets:UIEdgeInsetsMake(0.0, 2.0, 0.0, 2.0)];
...
- (CGRect)trackRect
{
    ...
    retValue.size = CGSizeMake(currentTrackImage.size.width, 5);
 ...
 -(CGRect) trackBackgroundRect
{
    ...
    trackBackgroundRect.size = CGSizeMake(_trackBackgroundImage.size.width, 5);
...
-(void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if (_upperHandle.highlighted ) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(updataValue:)]) {
            [self.delegate updataValue:self];
        }
    }
