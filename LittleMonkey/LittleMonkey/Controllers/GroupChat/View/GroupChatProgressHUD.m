...
@interface GroupChatProgressHUD ()
{
    NSTimer *myTimer;
    int angle;
...
@implementation GroupChatProgressHUD

@synthesize overlayWindow;

+ (GroupChatProgressHUD*)sharedView {
    static dispatch_once_t once;
    static GroupChatProgressHUD *sharedView;
    dispatch_once(&once, ^ {
        sharedView = [[GroupChatProgressHUD alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
...
+ (void)show {
    ...
    [[GroupChatProgressHUD sharedView] show];
    
}

+ (void)show:(NSString *)duration {
    
    [[GroupChatProgressHUD sharedView] show:duration];
...
- (void)show:(NSString *)duration {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (!edgeImageView)
            edgeImageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"icon_chat_record_circle"]];
        ...
        self.subTitleLabel.text = NSLocalizedString(@"手指上滑，取消发送", nil);
        ...
        self.titleLabel.text = NSLocalizedString(@"时间限制", nil);
...
{
    [[GroupChatProgressHUD sharedView] setState:str];
}
...
+ (void)dismissWithSuccess:(NSString *)str {
    [[GroupChatProgressHUD sharedView] dismiss:str];
}
...
- (void)dismiss:(NSString *)state {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        CGFloat timeLonger;
        if ([state isEqualToString:NSLocalizedString(@"说话时间太短了", nil)]) {
